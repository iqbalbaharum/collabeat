...
const AccentButton = ({ name, onClick, icon, className, disabled, color, textColor }: ButtonProp) => {
  return (
    <button
      disabled={disabled}
      onClick={onClick}
      className={`relative ring-1 ring-white/30 shadow-white inline-flex items-center justify-center px-2 py-1 overflow-hidden font-medium text-black transition duration-300 ease-out rounded-md group hover:ring-1 hover:ring-black ${
        className ?? ''
